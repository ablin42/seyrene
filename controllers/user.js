...
router.post("/lostpw", vLostPw, async (req, res) => {
  //check if pwtoken already exist
  try {
    ...
    if (user === null)
      throw new Error(
        "Invalid e-mail, please make sure you entered the correct e-mail"
      );
    ...
    if (pwToken === null) {
      ...
      var [err, result] = await utils.to(
        PwToken.updateOne({ _userId: user._id }, { $set: { token: token } })
      );
      if (err)
        throw new Error(
    ...
    req.flash(
      "success",
      "An e-mail was sent to your address, please follow the link we sent you"
    );
...
router.post("/patch/name", vName, verifySession, async (req, res) => {
  try {
    ...
    } else throw new Error("Unauthorized, please make sure you are logged in");
  } catch (err) {
...
router.post("/patch/email", vEmail, verifySession, async (req, res) => {
  try {
    if (req.user) {
      ...
      req.flash(
        "success",
        "Email successfully modified, please confirm your new e-mail by clicking on the link we sent you"
      );
      res.status(200).redirect("/User");
    } else throw new Error("Unauthorized, please make sure you are logged in");
  } catch (err) {
...
router.post("/patch/password", vPassword, verifySession, async (req, res) => {
  try {
    if (req.user) {
      ...
      var [err, user] = await utils.to(
        User.updateOne({ _id: id }, { $set: { password: hashPw } })
      );
      if (err)
        throw new Error(
          "An error occured while updating your password, please try again"
