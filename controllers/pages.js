...
router.get("/shopping-cart", setUser, authUser, setDelivery, isDelivery, async (req, res) => {
	try {
		...
		let obj = {
			...
			products: [],
			totalPrice: utils.parsePrice(cart.price.totalIncludingTax),
		...
		itemArr.forEach(item => {
			...
			if (item.attributes && item.isUnique) {
				itemObj = {
					...
					qty: item.qty,
					price: utils.parsePrice(item.price),
...
router.get("/Payment", setUser, authUser, setDelivery, isDelivery, checkBilling, async (req, res) => {
	try {
		...
		obj.totalPrice = utils.parsePrice(cart.price.totalIncludingTax);
	...
	try {
		...
		if (orders) {
			orders.forEach((order, index) => {
				orders[parseInt(index)].price = utils.parsePrice(order.price);
...
router.get("/Order/:id", setUser, authUser, setOrder, authGetOrder, async (req, res) => {
	try {
		let obj = {
			...
			user: req.user,
			order: req.order,
			deliveryPriceFormatted: utils.parsePrice(req.order.deliveryPrice)
		};
...
router.get("/Shop/:id", setUser, async (req, res) => {
	try {
		...
		obj.shop.price = utils.parsePrice(obj.shop.price);
